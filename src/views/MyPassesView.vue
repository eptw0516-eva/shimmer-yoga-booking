...
<template>
  <div class="animate-rise space-y-5">
    ...
    <section><div class="mb-3 flex items-center justify-between"><h2 class="font-display text-xl">即將到來</h2><span class="rounded-full bg-[#eef3ee] px-2.5 py-1 text-[11px] text-sage">{{ upcoming.length }} 堂</span></div>
      <div v-if="upcoming.length" class="space-y-3"><article v-for="booking in upcoming" :key="booking.id" class="rounded-3xl border border-sand bg-white p-4"><div class="flex items-start justify-between"><div><p class="mb-1 text-[11px] text-clay">{{ formatTaiwanDateTime(booking.class!.start_time) }}</p><h3 class="font-semibold">{{ booking.class!.title }}</h3></div><CalendarDays :size="19" class="text-sage" /></div><div class="mt-3 flex items-center gap-3 text-[11px] text-stone-500"><span class="flex items-center gap-1"><Clock3 :size="13" />{{ formatTaiwanDateTime(booking.class!.end_time) }} 結束</span><span>{{ booking.class!.instructor_name }} · {{ booking.class!.room }}</span></div><button class="mt-4 flex w-full items-center justify-center gap-1 border-t border-sand pt-3 text-xs text-stone-400" @click="cancel(booking)">取消預約 <ChevronRight :size="13" /></button></article></div>
      <div v-else class="rounded-3xl bg-white py-8 text-center text-sm text-stone-400">目前沒有即將到來的課程。</div>
    </section>
    <section><div class="mb-3 flex items-center justify-between"><h2 class="font-display text-xl">我的票券</h2><span class="text-xs text-sage">共 {{ store.packages.length }} 份</span></div><div v-if="!store.packages.length" class="rounded-3xl bg-white p-6 text-center text-sm text-stone-500">目前還沒有已核款票券。完成購課並由管理員確認入帳後，票券會顯示在這裡。</div><div v-else class="space-y-3"><article v-for="item in store.packages" :key="item.id" class="rounded-3xl border border-sand bg-white p-4"><div class="flex items-center gap-3"><span class="grid h-10 w-10 place-items-center rounded-2xl bg-[#f4ede7] text-clay"><Ticket :size="19" /></span><div class="flex-1"><p class="text-sm font-semibold">{{ item.package_name ?? item.plan_name ?? '瑜珈堂數方案' }}</p><p class="mt-1 text-[11px] leading-relaxed text-stone-500">{{ item.plan_description || '微光空中瑜珈練習方案' }}</p><p class="mt-1 text-[11px] text-stone-400">有效至 {{ formatTaiwanDateTime(item.valid_until) }} · {{ item.shareable ? '可共享給親友' : '限本人使用' }}</p></div><span class="text-xs font-medium text-sage">{{ item.status === 'active' ? '使用中' : item.status }}</span></div><div class="mt-4 grid grid-cols-2 gap-2 rounded-2xl bg-[#faf8f5] p-3 text-xs text-stone-500"><span>剩餘 <strong class="text-lg text-sage">{{ item.remaining_credits }}</strong> 堂</span><span>購買 <strong class="text-lg text-sage">{{ item.initial_credits ?? item.total_credits }}</strong> 堂</span><span>購買金額 <strong class="text-sage">NT$ {{ item.purchase_amount ?? '—' }}</strong></span><span>單堂金額 <strong class="text-sage">NT$ {{ item.unit_price ?? '—' }}</strong></span></div><div class="mt-3 flex justify-end"><button v-if="item.shareable || item.shared_with_phones?.length" class="flex items-center gap-1 rounded-xl bg-[#efeff7] px-3 py-2 text-xs font-semibold text-sage" @click="sharingPackage = item"><Users :size="14" />共享設定</button></div><div v-if="item.shared_with_phones?.length" class="mt-3 border-t border-sand pt-3 text-[11px] text-stone-400">共享親友：{{ item.shared_with_phones.join('、') }}</div></article></div></section>
