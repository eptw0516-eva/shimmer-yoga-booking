...
<template>
  <div class="animate-rise">
    ...
    <div class="space-y-3"><article v-for="profile in profiles" :key="profile.id" class="rounded-3xl border border-sand bg-white p-4"><div class="flex items-center gap-3"><span class="grid h-10 w-10 place-items-center rounded-full bg-[#e3e3ef] text-sage"><UserRound :size="18" /></span><div class="flex-1"><p class="text-sm font-semibold">{{ profile.full_name || '未設定姓名' }}</p><p class="mt-1 text-[11px] text-stone-400">{{ profile.phone || '未設定電話' }}</p><div class="mt-2 flex flex-wrap gap-1"><span class="rounded-full bg-[#efeff7] px-2 py-1 text-[10px] text-sage">{{ profile.role }}</span><span v-if="profile.is_instructor" class="rounded-full bg-[#f5ede9] px-2 py-1 text-[10px] text-clay">老師</span><span class="rounded-full px-2 py-1 text-[10px]" :class="profile.is_active ? 'bg-[#e8f3e9] text-sage' : 'bg-stone-100 text-stone-400'">{{ profile.is_active ? '啟用中' : '已停用' }}</span></div></div><button class="rounded-xl bg-sage px-3 py-2 text-xs font-semibold text-white" @click="edit(profile)">編輯</button></div></article></div>
