...
<template>
  <div class="animate-rise">
    ...
    <div class="space-y-3"><article v-for="plan in plans" :key="plan.id" class="rounded-3xl border border-sand bg-white p-4"><div class="flex items-start justify-between"><div><h2 class="font-semibold">{{ plan.name }}</h2><p class="mt-1 text-xs text-stone-500">{{ plan.description }}</p><p class="mt-2 text-[11px] text-stone-400">{{ plan.category }} · {{ plan.credits ?? '不限' }} 堂 · {{ plan.valid_days }} 天 · {{ plan.shareable ? '可共享' : '限本人' }}</p></div><strong class="text-sage">NT$ {{ plan.total_price }}</strong></div><div class="mt-3 flex gap-2 border-t border-sand pt-3"><button class="flex-1 rounded-xl bg-[#efeff7] py-2 text-xs text-sage" @click="edit(plan)">編輯</button><button class="flex-1 rounded-xl bg-[#f5ede9] py-2 text-xs text-clay" @click="toggle(plan)">{{ plan.is_active ? '停用' : '啟用' }}</button></div></article></div>
    <div v-if="editing" class="fixed inset-0 z-40 flex items-end justify-center bg-ink/30 p-4 sm:items-center"><section class="max-h-[90vh] w-full max-w-[398px] overflow-y-auto rounded-[28px] bg-cream p-5 shadow-xl"><h2 class="mb-4 font-display text-xl">設定購課方案</h2><div class="space-y-3"><input v-model="editing.id" placeholder="方案 ID" class="w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm" /><input v-model="editing.name" placeholder="方案名稱／課程主題" class="w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm" /><select v-model="editing.category" class="w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm"><option value="single">單堂</option><option value="credits">計次</option><option value="stored_value">儲值金</option><option value="unlimited">月費吃到飽</option></select><input v-model="editing.badge" placeholder="課程註記（初次體驗／最受歡迎）" class="w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm" /><textarea v-model="editing.description" placeholder="課程說明" class="w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm"></textarea><div class="grid grid-cols-2 gap-2"><input v-model.number="editing.valid_days" type="number" min="1" placeholder="有效天數" class="w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm" /><input v-model.number="editing.credits" type="number" min="0" placeholder="購買堂數" class="w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm" /></div><div class="grid grid-cols-2 gap-2"><input v-model.number="editing.unit_price" type="number" min="0" placeholder="單堂金額" class="w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm" /><input v-model.number="editing.total_price" type="number" min="0" placeholder="總價" class="w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm" /></div><label class="flex items-center gap-2 rounded-xl bg-white p-3 text-sm"><input v-model="editing.shareable" type="checkbox" /> 可共享給親友</label><label class="flex items-center gap-2 rounded-xl bg-white p-3 text-sm"><input v-model="editing.is_active" type="checkbox" /> 上架顯示</label></div><div class="mt-5 flex gap-2"><button class="flex-1 rounded-xl border border-sand py-3 text-sm" @click="editing = null">取消</button><button class="flex-1 rounded-xl bg-sage py-3 text-sm font-semibold text-white" @click="save">儲存方案</button></div></section></div>
