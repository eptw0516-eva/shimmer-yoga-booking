...
onMounted(() => { void loadInstructors(); void store.loadSchedule() })
</script>
...
<template>
  <div class="animate-rise">
    ...
    <template v-else><div class="mb-3 flex items-center justify-between"><h2 class="font-display text-xl">課程列表</h2><button class="rounded-xl bg-sage px-3 py-2 text-xs font-semibold text-white" @click="showClassForm = true; void loadInstructors()"><Plus :size="14" class="mr-1 inline" />新增課程</button></div><div class="space-y-3"><article v-for="item in store.classes" :key="item.id" class="flex items-center gap-3 rounded-3xl border border-sand bg-white p-4"><div class="grid h-10 w-10 place-items-center rounded-2xl bg-[#f4ede7] text-clay"><CalendarPlus :size="19" /></div><div class="flex-1"><p class="text-sm font-semibold">{{ item.title }}</p><p class="mt-1 text-[11px] text-stone-400">{{ item.instructor_name }} · {{ item.capacity }} 人上限</p></div><ChevronRight :size="17" class="text-stone-300" /></article></div></template>
