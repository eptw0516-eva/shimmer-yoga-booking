...
<template>
  <div class="animate-rise">
    ...
    <div class="mb-5 grid grid-cols-3 gap-1 rounded-2xl bg-sand p-1"><button class="rounded-xl py-2.5 text-xs transition" :class="activeTab === 'students' ? 'bg-white font-semibold text-sage shadow-sm' : 'text-stone-500'" @click="activeTab = 'students'"><UserRound :size="15" class="mr-1 inline" />學員管理</button><button class="rounded-xl py-2.5 text-xs transition" :class="activeTab === 'classes' ? 'bg-white font-semibold text-sage shadow-sm' : 'text-stone-500'" @click="activeTab = 'classes'"><CalendarPlus :size="15" class="mr-1 inline" />排課管理</button><button class="rounded-xl py-2.5 text-xs text-stone-500" @click="router.push('/admin/orders')"><ReceiptText :size="15" class="mr-1 inline" />購課核帳</button></div>
