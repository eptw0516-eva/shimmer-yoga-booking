...
async function saveClass() {
  ...
  if (await store.updateClass(editingClass.value.id, { start_time: new Date(editingClass.value.start_time).toISOString(), end_time: new Date(editingClass.value.end_time).toISOString(), capacity: editingClass.value.capacity })) editingClass.value = null
}
onMounted(() => { void loadInstructors(); void store.loadSchedule(); void loadStudents() })
</script>
...
<template>
  <div class="animate-rise">
    ...
    <template v-else><div class="mb-3 flex items-center justify-between"><h2 class="font-display text-xl">課程列表</h2><button class="rounded-xl bg-sage px-3 py-2 text-xs font-semibold text-white" @click="showClassForm = true; void loadInstructors()"><Plus :size="14" class="mr-1 inline" />新增課程</button></div><div class="space-y-3"><article v-for="item in store.classes"     :key="item.id" class="flex items-center gap-3 rounded-3xl border border-sand bg-white p-4" @click="editClass(item)"><div class="grid h-10 w-10 place-items-center rounded-2xl bg-[#f4ede7] text-clay"><CalendarPlus :size="19" /></div><div class="flex-1"><p class="text-sm font-semibold">{{ item.title }}</p><p class="mt-1 text-[11px] text-stone-400">{{ item.instructor_name }} · {{ item.capacity }} 人上限</p></div><ChevronRight :size="17" class="text-stone-300" /></article></div></template>
    ...
    <div v-if="editingClass" class="fixed inset-0 z-[100] flex items-end justify-center bg-ink/30 p-4 sm:items-center"><section class="w-full max-w-[398px] rounded-[28px] bg-cream p-5 shadow-xl"><div class="mb-4 flex items-center justify-between"><h2 class="font-display text-xl">修改單堂課程</h2><button @click="editingClass = null"><X :size="18" /></button></div><p class="mb-4 text-sm font-semibold">{{ editingClass.title }}</p><div class="space-y-3"><label class="block text-xs text-stone-500">上課開始時間<input v-model="editingClass.start_time" type="datetime-local" class="mt-1 w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm" /></label><label class="block text-xs text-stone-500">上課結束時間<input v-model="editingClass.end_time" type="datetime-local" class="mt-1 w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm" /></label><label class="block text-xs text-stone-500">人數上限<input v-model.number="editingClass.capacity" type="number" min="1" class="mt-1 w-full rounded-xl border border-sand bg-white px-3 py-2.5 text-sm" /></label></div><div class="mt-5 flex gap-2"><button class="flex-1 rounded-xl border border-sand py-3 text-sm" @click="editingClass = null">取消</button><button class="flex-1 rounded-xl bg-sage py-3 text-sm font-semibold text-white" @click="saveClass">儲存修改</button></div></section></div>
