-- Run after 006. This is safe to run repeatedly and repairs legacy tables
-- that were created with only a subset of the current application columns.
...
begin
  if not exists (
    ...
    alter table public.user_packages add constraint user_packages_user_id_fkey
...
do $$
begin
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'user_packages' and policyname = 'members read their packages') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'user_packages' and policyname = 'staff manage packages') then
